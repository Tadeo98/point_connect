...
#######################################################################
## CESTY A NAZVY
point_layer_path = r"D:\Praca\SAHI\Kosice\geodezia\plan_pre_mirku\data2\L1_tade_luka_mirka_13_9_2022.shp"   #cesta k bodovej vrstve
output_folder = r"D:\Praca\SAHI\Kosice\geodezia\plan_pre_mirku\data2\script_outds"  #cesta k priecinku, kde sa ulozi vysledok
...
identic_points_check = 1    #kontrola zamerania identickych bodov dvakrat vramci jedneho prvku (v premennych nastavenie identic_points_distance), 0 = bez kontroly, 1 = s kontrolou a oznamenim kodu prvku, kde je potvrdena identickost, 2 = s kontrolou, oznamenim a vymazanim prveho z dvojice identickych bodov (druhe meranie sa povazuje za spravne)


## PREMENNE
include_features = ('OBJ','HROB')   #zadanie retazcov kodov/casti kodov prvkov, ktore budu vo vystupe. Napr. ('OBJ','HROB','Obj','H')
exclude_features = ('VB','kera','FTG','SHL','bronz','Foto','foto','rez','SON','Tele','PROF','Rez','find')   #zadanie retazcov kodov/casti kodov prvkov, ktore nebudu vo vystupe. Napr. ('VB','kera','FTG','SHL','PROF')
    ...
    if exclude_count == 0 and include_count > 0 or include_features == ():
        ...
        try:
            ...
            if feature_type == 1:
                ...
                feature_polygon = feature = None
            feature_point_count = 0
            print(err, "\nError kvoli poslednemu prvku vrstvy s ID:", point_number+1)
            break
if warn_count == 0:
